...
def event_generator(doctor,temps):
    ...
    if 0<=temps<480:              #morning between 0000 and 0800
       ...
       if p <= 0.6:
           t=temps
           doctor='nursing'    #60% to be in consultation
           temps+=30
           return doctor, temps, temps-t
        ...
        p=rd.random()
        if doctor=='surgery':       # pause after surgery if lunchpause 
            doctor='pause'
            t=temps
            temps+=30
            return doctor, temps, temps-t
...
fichier.write("(assert (job nurse))")
fichier.close
...
while d<1:
    ...
    if result[0] =='nursing':
        fichier = open("commandflow", "a")
        fichier.write("(assert (stop nursing))\n")
        fichier.close
        print('stop nursing')
    elif result[0] =='surgery':
        fichier = open("commandflow", "a")
        fichier.write("(assert (stop surgery))\n")
        fichier.close
        print('stop surgery')
